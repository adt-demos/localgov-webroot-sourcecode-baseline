...
class ScheduledTransitionAddForm extends ContentEntityForm {
  ...
  public static function create(ContainerInterface $container): static {
    return new static(
      ...
      $container->get(ModerationInformationInterface::class),
      $container->get(StateTransitionValidationInterface::class),
      $container->get(LanguageManagerInterface::class),
      $container->get(ScheduledTransitionsUtilityInterface::class),
  ...
  public function form(array $form, FormStateInterface $form_state): array {
    ...
    if (\is_numeric($revision) && $revision > 0) {
      ...
      /** @var \Drupal\Core\Entity\ContentEntityInterface&\Drupal\Core\Entity\RevisionableInterface $entityRevision */
    ...
    elseif (\is_string($revision)) {
      ...
      $toTransitions = \array_filter(
        ...
        static fn (TransitionInterface $transition) => $account->hasPermission('use ' . $workflow->id() . ' transition ' . $transition->id()),
      );
    }
    ...
    if (isset($toTransitions) && \count($toTransitions) > 0) {
      ...
      foreach ($toTransitions as $transition) {
        $to = $transition->to();
        if (!$to instanceof ContentModerationState) {
          throw new \LogicException('Transitions should only be a ' . ContentModerationState::class);
        }

        if ($to->isDefaultRevisionState()) {
    ...
    if ($entity instanceof RevisionLogInterface && ($entityRevision === NULL || $entityRevision instanceof RevisionLogInterface)) {
   ...
   */
  public function ajaxCallbackNewMeta(array $form, FormStateInterface $form_state): array {
  ...
  public function validateForm(array &$form, FormStateInterface $form_state): ContentEntityInterface {
    if (!\is_string($form_state->getValue('revision'))) {
      $form_state->setError($form['scheduled_transitions']['revision'], $this->t('Revision must be selected.'));
    }
    // Drupal's current typing for this prop is using `|` when it should be using `&`:
  ...
  public function submitForm(array &$form, FormStateInterface $form_state): void {
    ...
    if ($form_state->getValue('recreate_non_default_head') === 1) {
  ...
  protected function getRevisionOptions(EntityInterface $entity): array {
    ...
    $workflowPlugin = $this->moderationInformation->getWorkflowForEntity($entity)?->getTypePlugin();
    /** @var \Drupal\content_moderation\ContentModerationState[] $workflowStates */
    ...
    $entityRevisions = \array_map(function (int|string $revisionId) use ($entityStorage): EntityInterface {
      // Ignore until https://www.drupal.org/project/drupal/issues/3390366
    ...
    $entityRevisions = \array_filter(
      $entityRevisions,
      static fn (EntityInterface $revision) => $revision instanceof TranslatableRevisionableInterface ? $revision->isRevisionTranslationAffected() : TRUE,
    );

    $options = \array_map(
      function (RevisionableInterface $entityRevision) use ($workflowStates): array {
        $option = [];
        $revisionTArgs = ['@revision_id' => $entityRevision->getRevisionId()];

        // Don't add the arg to toLink in case this particular entity has
        ...
        $option['state']['data'] = $moderationState?->label() ?? $this->t('- Unknown state -');
   ...
   * @param \Drupal\Core\Entity\RevisionLogInterface|null $entityRevision
   *   The revision being transitioned, if available.
   *
   * @return array
   *   A form array.
   */
  protected function getRevisionMetadata(FormStateInterface $form_state, ?WorkflowInterface $workflow, RevisionLogInterface $entity, ?RevisionLogInterface $entityRevision): array {
    $form = [
      '#tree' => TRUE,
    ];
    ...
    /** @var string|null $transitionValue */
