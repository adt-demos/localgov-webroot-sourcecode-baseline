...
Since Drupal 11.3, the field storage for node body is no longer included. We
depend on that field so include the ```field.storage.node.body.yml``` config
file in the 3.x branch of localgov_core.
