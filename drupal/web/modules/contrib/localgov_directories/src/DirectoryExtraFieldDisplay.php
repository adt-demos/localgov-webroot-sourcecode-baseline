...
class DirectoryExtraFieldDisplay implements ContainerInjectionInterface, TrustedCallbackInterface {
  ...
  public function groupDirFacetItems(array $facet_items): array {
    ...
    foreach ($facet_items as $key => $item) {
      $facet_ids[] = $item['value']['#attributes']['data-drupal-facet-item-value'] ?? $key;
    }
    $facet_entities = $facet_storage->loadMultiple($facet_ids);

    foreach ($facet_items as $key => $item) {
      $facet_id = $item['value']['#attributes']['data-drupal-facet-item-value'] ?? $key;
      if (isset($facet_entities[$facet_id])) {
        $facet_entity = $facet_entities[$facet_id];
    ...
    $facet_type_entities = $type_storage->loadMultiple(array_keys($group_items));

    foreach ($group_items as $bundle => $items) {
      if (isset($facet_type_entities[$bundle])) {
        $facet_type_entity = $facet_type_entities[$bundle];
    ...
    // The following comment helps static analysis tools understand that the
    // weight and title keys will be set on the group items at this point.
    // It was generated with the help of Claude Code.
    /** @var array<string, array<string, mixed>> $group_items */
