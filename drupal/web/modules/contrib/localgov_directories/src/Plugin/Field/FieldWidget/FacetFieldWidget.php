 ...
 * )
 */
class FacetFieldWidget extends OptionsWidgetBase implements ContainerFactoryPluginInterface {
  ...
  public function formElement(FieldItemListInterface $items, $delta, array $element, array &$form, FormStateInterface $form_state) {
    ...
    // it back for now.
    $raw_options = $this->selectionPluginManager->getSelectionHandler($this->fieldDefinition, $items->getEntity())->getReferenceableEntities();
    ...
    if ($user_input = $form_state->getValue('localgov_directory_channels')) {
      foreach ($user_input as $user_input_nid) {
        if ($user_input_nid['target_id'] && ($channel = $this->entityTypeManager->getStorage('node')->load($user_input_nid['target_id']))) {
